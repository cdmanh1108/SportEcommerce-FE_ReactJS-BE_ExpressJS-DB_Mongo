...
const SidebarSortComponent = ({ isOpen, onClose, onFilterChange }) => {
    ...
    if (categories.length > 0) {
      ...
      categories.forEach((cat) => {
        if (
          ...
          !seenCategoryNames.has(cat.category_name)
        ) {
          seenCategoryNames.add(cat.category_name);
          groupedCategories[cat.category_name] = {
            name: cat.category_name,
            ids: [cat._id],
          };
        } else if (cat.category_level === 1) {
          groupedCategories[cat.category_name].ids.push(cat._id);
        }
      });
      ...
      categories.forEach((cat) => {
        if (
          ...
          !seenSubCategoryNames.has(cat.category_name)
        ) {
          seenSubCategoryNames.add(cat.category_name);
          groupedSubCategories[cat.category_name] = {
            name: cat.category_name,
            ids: [cat._id],
            parentId: cat.category_parent_id,
          };
        } else if (cat.category_level === 2) {
          groupedSubCategories[cat.category_name].ids.push(cat._id);
        }
      });
  ...
  useEffect(() => {
    if (
      ...
      const selectedCategoryIds = categories
        ?.filter(
          (cat) =>
            selectedFilters?.category?.includes(cat.category_name) &&
            cat.category_level === 1,
        )
        ?.map((cat) => cat._id);
  ...
  const handleSelect = (type, value) => {
    setSelectedFilters((prev) => {
      ...
      if (
        ["category", "category_sub", "product_color", "product_brand"].includes(
          type,
        )
      ) {
        if (type === "category" && isSelected) {
          const newCategoryList = currentList.filter((item) => item !== value);
          // Khi bỏ chọn category, clear category_sub vì sub options sẽ được filter lại
          return {
            ...prev,
