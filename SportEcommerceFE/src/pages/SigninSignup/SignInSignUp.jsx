...
const SignInSignUp = () => {
  ...
  const handleSubmitSignUp = async (e) => {
    ...
    const result = await handleSendSignUpOTP(
    ...
    if (result?.EC === 0) {
      ...
      showPopup(result.EM || "Đã gửi mã xác thực về email");
    } else {
  ...
  const handleResendSignUpOtp = async () => {
    ...
    if (result?.EC === 0) {
      setOtpError("");
      showPopup(result.EM || "Đã gửi lại mã xác thực");
    } else {
      setOtpError(result?.EM || "Gửi lại mã xác thực thất bại");
    }
  };

  const handleVerifySignUpOtp = async (otpArray) => {
    const enteredCode = otpArray.join("").trim();
    ...
    if (!pendingSignUpData?.email) {
      setOtpError("Thiếu dữ liệu đăng ký, vui lòng thử lại");
      return;
    }
    ...
    if (result?.EC === 0) {
      ...
      resetSignUpForm();
      setIsSignUp(false);
      showPopup(result.EM || "Đăng ký thành công");
      navigate("/sign-in");
    } else {
      setOtpError(result?.EM || "Mã xác thực không hợp lệ hoặc đã hết hạn");
    }
  };
  ...
  return (
    <div className="bg-white py-14 px-2">
      ...
      <Dialog open={isOtpDialogOpen} handler={closeOtpDialog} size="xs">
        <DialogHeader className="pb-0">Xác thực đăng ký</DialogHeader>
        <DialogBody className="pt-2">
          <OTPComponent
            newEmail={pendingSignUpData?.email || signUpEmail}
            otpError={otpError}
        ...
        <DialogFooter className="pt-0">
          <Button variant="text" color="gray" onClick={closeOtpDialog}>
            Đóng
          </Button>
        </DialogFooter>
      </Dialog>
