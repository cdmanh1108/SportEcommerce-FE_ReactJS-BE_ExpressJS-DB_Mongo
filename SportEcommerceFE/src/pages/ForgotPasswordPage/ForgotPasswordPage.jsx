...
const ForgotPasswordPage = () => {
  ...
  const handleSendOtp = async () => {
    ...
    if (res?.EC === 0) {
      setStep(1);
    } else if (normalizeText(res?.EM).includes("khong tim thay nguoi dung")) {
      setError("Người dùng không tồn tại");
    } else {
      setError(res?.EM || "Gửi mã OTP thất bại, vui lòng thử lại");
    }
    setLoading(false);
  };
  ...
  const handleVerifyOtp = async (otpArray) => {
    ...
    if (res?.EC === 0) {
      setStep(2);
    } else if (normalizeText(res?.EM).includes("otp khong hop le")) {
      setOtpError("Mã OTP không hợp lệ");
    } else {
      setOtpError(res?.EM || "Xác thực OTP thất bại");
    }
  };
  ...
  const handleResetPasswordSubmit = async () => {
    ...
    if (res?.EC === 0) {
      showPopup(res?.EM || "Đổi mật khẩu thành công. Vui lòng đăng nhập lại", true, 1000);
      navigate("/sign-in");
    } else {
      showPopup(res?.EM || "Đặt lại mật khẩu thất bại, vui lòng thử lại", false);
    }
  };
